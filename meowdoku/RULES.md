# Meowdoku: the rules, illustrated

Each rule comes with a picture in the game's own style, a plain-English statement, a formal statement, and a proof.  The Z3 checks referenced are in `results.json` (produced by `meowdoku_z3.py`); pictures are produced by `make_images.py`; this file by `make_rules.py`.
...
* [What one cat rules out](#rules)
* [Rule 1: common attack](#common)
* [Rule 2: the knight's move](#knight)
* [Rule 3: how many cats fit in a rectangle](#capacity)
* [Rule 4: the 3 x 3 block](#three)
* [Rule 5: two colours in a 2 x 3](#twobythree)
* [Rule 6: three colours in a 3 x 4](#threebyfour)
* [Rule 7: four colours in a 4 x 4, the knight ring](#fourbyfour)
* [Rule 8: colours locked inside rows](#hallrows)
* [Rule 9: rows made of few colours](#hallregions)
* [Rule 10: a full rectangle keeps other colours out](#boxcolour)
* [Rule 11: a rectangle that cannot be empty](#lower)
* [Rule 12: one what-if is enough (locally)](#whatif)
* [Shape cheat sheet](#shapes)
* [Worked example: level 155](#level)

<a id="rules"></a>
## What one cat rules out

![What one cat rules out](img/fig00_rules.png)

**In plain English.** Placing a cat crosses out its whole row, its whole column, the rest of its colour, and the eight cells around it. Everything else in this document is a way of crossing out cells *before* you know where the cat is.

**Formally.**

```
x[a] = 1  implies  x[b] = 0  for every b with a -> b,
...
![Rule 1: common attack](img/fig01_common_attack.png)

**In plain English.** Look at where a colour's cat could still be. If every one of those cells would cross out some cell c, then c is crossed out now, without knowing which of them is right. The same works for the remaining cells of a row or a column. Typical shapes: an L crosses out its inner corner, a bar of three crosses out the cells above and below its middle, a bar of two crosses out the six cells above and below it.

**Formally.**

```
Let U be a unit whose cat is known to lie in K (a set of cells), and let c be
...
![Rule 2: the knight's move](img/fig02_knight.png)

**In plain English.** The cats of two neighbouring rows are always at least two columns apart, like a knight's jump or wider. So once a row's cat is pinned to a few columns, the next row loses those columns *and* the columns right beside them. Neighbouring columns behave the same way.

**Formally.**

```
If x[r,c] = 1 and x[r+1,c'] = 1 then |c - c'| >= 2.  Equivalently, a 2 x 2 block
...
<a id="capacity"></a>
## Rule 3: how many cats fit in a rectangle

![Rule 3: how many cats fit in a rectangle](img/fig03_capacity_small.png)

**In plain English.** A rectangle a cells tall and b cells wide never holds more than min(a, b) cats, and two sizes are worse than that: a 2 x 2 holds at most one cat, and a 3 x 3 holds at most two. So if you ever see two colours entirely inside a 2 x 2, three colours inside a 3 x 3, or three colours inside two rows, you have misread the board: the puzzle would have no solution.

**Formally.**

```
cap(a, b) = min(a, b) for all a, b >= 1, except cap(2, 2) = 1 and cap(3, 3) = 2.
...
<a id="three"></a>
## Rule 4: the 3 x 3 block

![Rule 4: the 3 x 3 block](img/fig04_3x3.png)

**In plain English.** Two cats in a 3 x 3 block use up the block: the third row and column of the block get their cats from outside. And if two colours are confined to a 3 x 3, its centre cell is empty, because a cat in the centre would leave no room for the second one.

**Formally.**

```
If two colours are confined to a 3 x 3 block B with centre m, then x[m] = 0.
...
<a id="twobythree"></a>
## Rule 5: two colours in a 2 x 3

![Rule 5: two colours in a 2 x 3](img/fig05_2x3.png)

**In plain English.** If two colours fit inside a 2 x 3 block, the cats are in the two end columns of the block, one per row, so the middle column of the block is empty and so are the cells directly above and below it. Both rows and both end columns are then spoken for: everything else in them is empty.

**Formally.**

```
Let B be a 2 x 3 block with rows r, r+1 and columns c, c+1, c+2, and let two
...
<a id="threebyfour"></a>
## Rule 6: three colours in a 3 x 4

![Rule 6: three colours in a 3 x 4](img/fig06_3x4.png)

**In plain English.** Three colours inside a 3 x 4 block put the middle row's cat at one of the two ends of the block, so the two inner cells of that row are empty. Both end columns of the block are used, and all three rows are used, so everything outside the block in those rows and columns is empty too.

**Formally.**

```
Let B be a 3 x 4 block with rows r..r+2 and columns c..c+3, and three colours
...
<a id="fourbyfour"></a>
## Rule 7: four colours in a 4 x 4, the knight ring

![Rule 7: four colours in a 4 x 4, the knight ring](img/fig07_4x4.png)

**In plain English.** Four colours inside a 4 x 4 block leave exactly two ways to place the cats, and both are the same ring of knight's moves. The four corners of the block and its central 2 x 2 are empty, and all four rows and columns of the block are used, so nothing outside the block in those rows and columns can hold a cat.

**Formally.**

```
Let B be a 4 x 4 block with four colours confined to it.  Then the cats in B are
...
<a id="hallrows"></a>
## Rule 8: colours locked inside rows

![Rule 8: colours locked inside rows](img/fig08_hall_rows.png)

**In plain English.** If some k colours live entirely inside the same k rows, those rows have no cats to spare: every cell of those rows that belongs to another colour is empty. The same holds with columns instead of rows, and with a rectangle instead of rows (Rule 10).

**Formally.**

```
Let S be a set of k rows and let colours G_1..G_k be confined to the rows in S.
...
<a id="hallregions"></a>
## Rule 9: rows made of few colours

![Rule 9: rows made of few colours](img/fig09_hall_regions.png)

**In plain English.** Turn Rule 8 around. If k rows contain cells of only k colours, then those k colours must place their cats inside those rows, so any cell of those colours lying outside the rows is empty. Again the same holds for columns.

**Formally.**

```
Let S be a set of k rows such that every cell in the rows of S belongs to one of
...
<a id="boxcolour"></a>
## Rule 10: a full rectangle keeps other colours out

![Rule 10: a full rectangle keeps other colours out](img/fig10_box_colour.png)

**In plain English.** If as many colours as a rectangle can hold are confined to it, the rectangle is full: any cell inside it that belongs to a different colour is empty. The rectangle sizes that matter in practice are the 2 x 2 (one colour), the 2 x 3 and 3 x 3 (two colours), the 3 x 4 (three) and the 4 x 4 (four).

**Formally.**

```
Let B be an a x b rectangle and let cap(a, b) colours be confined to B.  Then
...
![Rule 11: a rectangle that cannot be empty](img/fig11_lower_bound.png)

**In plain English.** Plain counting says a block of a rows and b columns must contain at least a + b - N cats. Touching makes this stronger: the cats of those a rows that are not in the block have to fit in the leftover columns, which is itself a rectangle with a capacity. For example the two cats of rows 1-2 cannot both sit in two adjacent leftover columns, so a 2 x (N-2) block at the edge always contains a cat.

**Formally.**

```
Let R be a set of rows and C a set of columns, with C' the remaining columns and
...
<a id="whatif"></a>
## Rule 12: one what-if is enough (locally)

![Rule 12: one what-if is enough (locally)](img/fig12_whatif.png)

**In plain English.** Some cells can only be crossed out by trying them: put an imaginary cat there, cross out what it attacks, and watch a nearby colour get squeezed to a single cell that in turn takes the last cell of another colour. Z3 checked every pair of small colours inside a 3 x 3 window: whenever the direct rules miss something, a single what-if with the direct rules finds it, and a single what-if is never wrong.

**Formally.**

```
Assume x[c] = 1, apply Rules 0-11 to a fixed point, and suppose some unit is
...
![Shape cheat sheet](img/fig13_shapes.png)

![Shape cheat sheet (continued)](img/fig13_shapes_b.png)

**In plain English.** The shapes whose crossed-out cells are easy to miss. Every one is an instance of Rule 1. Straight bars of four or more, the 2 x 2 square, and the P, L, Y, Z and X pentominoes cross out nothing beyond their own row or column.

**Formally.**

```
For a colour G of a given shape, the forced-empty set is
...
<a id="level"></a>
## Worked example: level 155

![Worked example: level 155](img/fig14_level155.png)

**In plain English.** The screenshot position. Column 5 (counting from 1) has a single open cell, so the blue cat goes there. It touches the rose cell below-right, so rose goes to its other cell. That takes column 8, so green goes to its remaining cell in row 6, and brown's column then has one free row left.

**Formally.**

```
With the six placed cats fixed, the four cells (5,5), (6,1), (7,4), (8,7)
